   ...
   <?php
     ...
     if(isset($_SESSION["userid"])){
       ...
       "SELECT * FROM useraccounts
          ...
          where useraccounts.email = '".$_SESSION['useraccount_email']."' and projects.type='Loan';";
       ...
       $_SESSION['$sql_get_deals'] = "SELECT * FROM useraccounts
					...
					JOIN projects ON customer_projects.project_id = projects.project_id
                    join buildings on buildings.building_id = projects.building_id
					where useraccounts.email = '".$_SESSION['useraccount_email']."' and projects.type='Deal';";
     }
   ?>
...
/////// query is above the HTML document
    ...
    if ($number_of_rows){
      while ($row = mysqli_fetch_array($query_loans))
      {
        ...
        $row['number'] .'<br>'.  $row['zipcode'] ." ".  utf8_encode($row['city']) . '<br><br><a href=projects.php>See project details</a>");
        ';
      }
    }
